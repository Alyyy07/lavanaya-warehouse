<x-app-layout>
    ...
    <div class="card-app mx-auto p-0 overflow-hidden max-w-4xl">
        <div class="bg-primary px-8 py-4 flex justify-between items-center text-white">
            <h2 class="text-xs font-black tracking-[0.2em]">Registrasi Barang Keluar</h2>
        </div>

        <form action="{{ route('transactions.out.store') }}" method="POST" class="p-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div class="space-y-6">
                    ...
                    <div>
                        <label class="block text-xs font-black tracking-widest text-slate-500 mb-2">Destinasi <span
                                class="text-red-600">*</span></label>
                        <input type="text" name="destination" class="input-app w-full text-xs font-black"
                            placeholder="Cabang / Gudang / Client Receiver" value="{{ old('destination') }}" required />
                        @error('destination')
                        <p class="mt-2 text-xs font-black text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <select name="product_id" class="input-app w-full text-xs font-black" required>
                            ...
                            <option value="{{ $product->id }}" {{ old('product_id')==$product->id ? 'selected' : '' }}>
                                {{ $product->code }} - {{ $product->name }} (AVAIL: {{ $product->current_stock }} {{
                                $product->unit }})
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <label class="block text-xs font-black tracking-widest text-slate-500 mb-2">Volume
                            Pengeluaran <span class="text-red-600">*</span></label>
                        <div class="relative">
                            <input type="number" name="quantity"
                                class="input-app w-full text-xs font-black text-red-600 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            ...
            <div class="mt-10">
                ...
                <textarea name="notes" class="input-app w-full min-h-25 text-xs font-bold py-3"
                    placeholder="Catatan Distribusi">{{ old('notes') }}</textarea>
                @error('notes')
                <p class="mt-2 text-xs font-black text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-12 pt-8 border-t-2 border-border flex justify-end gap-4">
                ...
                <button type="submit"
                    class="btn-app-primary px-12 py-3 text-xs font-black tracking-[0.2em] bg-red-600 border-red-600 hover:bg-red-700 hover:border-red-700">
                    Simpan Data
                </button>
            </div>
        </form>
